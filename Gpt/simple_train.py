...
from text_fuc import text_to_token_ids , token_ids_to_text , generate_and_sample  , generate , real_time_generation
...
def generate_and_print_sample(model , tokenizer , device , start_context ):
    ...
    with torch.no_grad():
        ...
        token_ids = generate(model=model , idx=text_to_token_ids("Every effort moves you", tokenizer) 
                             , temperature=1.4 , max_new_tokens=15 ,context_size=126  , top_k=25 )
...
def train_model(model:nn.Module , train_dataloader:torch.utils.data.DataLoader ,  device:torch.device  ,
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        # real_time_generation(model, initial_input=[1, 2, 3, 4], context_size=100, temperature=0.7, top_k=10)
